...
<?php
    ...
    if($vars['entity']){
        ...
        foreach($vars['entity'] as $report){
            ...
            if($report->state != 'archived') {
	    	  $url =  elgg_validate_action_url($vars['url'] . "action/reportedcontent/archive?item=" . $report->guid );
            	  echo "<a class=\"archive_report_button\" href=\"$url\">" . elgg_echo('reportedcontent:archive') . "</a>";
            }
	    $url = elgg_validate_action_url($vars['url'] . "action/reportedcontent/delete?item=" . $report->guid);
